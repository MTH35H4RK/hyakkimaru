...
void setTarget(float t, float deltaT) {
  ...
  float pulsesPerMeter = 350 * 7.38;

  if (t < travelTime) {
    positionChange[0] = distance1 / travelTime * deltaT * pulsesPerMeter ;
    positionChange[1] = distance1 / travelTime * deltaT * pulsesPerMeter ;
  } else {
    MOVE = false;
  }
  ...
  while (true) {
    if (Serial.available() > 0) {
      ...
      if (input.startsWith("M5")) {
        ...
        distance1 = distance / 100.0;
        Serial.print("in m: ");
        Serial.println(distance1);
        // Calculate the travel time based on the distance
        travelTime = distance1 / 0.2; // 1 second for every 0.20 units of distance
        Serial.print("TT ");
